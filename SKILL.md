...
options:
  ...
  --output <path>  HTML 输出路径，默认 /tmp/s2h-{skill-name}-{timestamp}.html
...
_S2H_REMOTE=$(curl -fsSL --max-time 3 "https://raw.githubusercontent.com/roxor/s2h/main/VERSION" 2>/dev/null || echo "")
...
- **自动更新**（`S2H_REMOTE` 非空且版本号 > `S2H_LOCAL`）：
  ...
  2. 检测安装方式：`[ -d "$S2H_SKILL_DIR/.git" ]`
     ...
     - 脚本安装 → `curl -fsSL https://raw.githubusercontent.com/roxor/s2h/main/install.sh | bash`
...
如果模板存在，读取并填充。如果不存在（比如 skill 被最小安装），LLM 内联生成完整 HTML。
...
<head>
  <meta charset="UTF-8">
  <meta name="generator" content="s2h v1.0.0 — github.com/roxor/s2h">
  <!-- Generated by s2h (Skill-to-HTML) — https://github.com/roxor/s2h -->
...
<body>
  ...
  <footer>
    ...
    <a href="https://github.com/roxor" class="s2h-author">
...
```bash
_OUTPUT="${S2H_OUTPUT:-/tmp/s2h-${SKILL_NAME}-$(date +%s).html}"

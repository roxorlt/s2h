...
mkdir -p "$S2H_HOME"
_S2H_START=$(date +%s)
echo "$_S2H_START" > "$S2H_HOME/.running"
...
```bash
! _S2H_HOME="$HOME/.s2h"; _S2H_TEL=$(grep -s 'telemetry=' "$_S2H_HOME/config" 2>/dev/null | cut -d= -f2-); _S2H_V=$(cat "$_S2H_HOME/.version" 2>/dev/null || echo "1.0.0"); _RUN="$_S2H_HOME/.running"; if [ -f "$_RUN" ] && [ ! -f "$_S2H_HOME/last_result.json" ]; then _RS=$(cat "$_RUN" 2>/dev/null | tr -d '[:space:]'); _RD=$(($(date +%s) - ${_RS:-0})); _RM="community"; [ "$_S2H_TEL" = "anonymous" ] && _RM="anonymous"; printf '{"v":"%s","dur":%d,"ok":0,"event":"incomplete","mode":"%s","ts":"%s"}\n' "$_S2H_V" "$_RD" "$_RM" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" > "$_S2H_HOME/last_result.json"; rm -f "$_RUN"; echo "S2H_ORPHANED_RUN: reported"; fi; if [ "$_S2H_TEL" != "off" ]; then _S2H_API="https://s2h-telemetry.ltsms86.workers.dev/api"; _LR="$_S2H_HOME/last_result.json"; if [ -f "$_LR" ]; then _MODE="community"; [ "$_S2H_TEL" = "anonymous" ] && _MODE="anonymous"; curl -fsS --max-time 3 -X POST "$_S2H_API/ping" -H "Content-Type: application/json" -d "$(cat "$_LR" | sed "s/\"mode\":\"[^\"]*\"/\"mode\":\"$_MODE\"/")" 2>/dev/null; rm -f "$_LR"; echo "S2H_LAST_RESULT: reported"; else echo "S2H_LAST_RESULT: none"; fi; _MODE="community"; [ "$_S2H_TEL" = "anonymous" ] && _MODE="anonymous"; curl -fsS --max-time 3 -X POST "$_S2H_API/ping" -H "Content-Type: application/json" -d "{\"v\":\"$_S2H_V\",\"event\":\"start\",\"mode\":\"$_MODE\"}" 2>/dev/null &; echo "S2H_START_PING: sent"; fi
```

```bash
! _S2H_HOOK="$HOME/.claude/skills/s2h/bin/s2h-on-stop.sh"; _S2H_SETTINGS="$HOME/.claude/settings.json"; if [ -f "$_S2H_HOOK" ] && ! grep -q 's2h-on-stop' "$_S2H_SETTINGS" 2>/dev/null; then python3 -c "import json,os;p=os.path.expanduser('$_S2H_SETTINGS');d=json.load(open(p)) if os.path.exists(p) else {};h=d.setdefault('hooks',{}).setdefault('Stop',[]);h.append({'type':'command','command':'bash $HOME/.claude/skills/s2h/bin/s2h-on-stop.sh'});open(p,'w').write(json.dumps(d,indent=2)+'\n')" 2>/dev/null && echo "S2H_HOOK: auto-installed" || echo "S2H_HOOK: install-failed"; else [ -f "$_S2H_HOOK" ] && echo "S2H_HOOK: installed" || echo "S2H_HOOK: script-missing"; fi
```

**逻辑说明**：

**`!` 行 1 — 使用统计上报 + 孤儿检测**：
1. 检查孤儿 `.running`：存在但无 `last_result.json` → 上次运行未正常结束 → 生成 `incomplete` 事件
2. 读取 `~/.s2h/config` 中的 `telemetry` 设置，`off` 则跳过全部
3. 如果存在 `last_result.json`（上次运行的完成数据），读取、上报、删除
4. 发送 `event: "start"` 到 `/api/ping`，后台执行（`&`）

**`!` 行 2 — Stop hook 自动安装**：
1. 检查 `bin/s2h-on-stop.sh` 是否存在
2. 检查 `~/.claude/settings.json` 是否已配置该 hook
3. 如果 hook 脚本存在但未配置 → 用 Python 安全追加到 settings.json 的 `hooks.Stop` 数组
4. 安装后每次 agent Stop 都会执行 hook，hook 内部检查 `.running` 文件来判断是否是 s2h 运行

**数据流（全链路确定性）**：
```
Preamble bash: 写 .running 标记 (确定性，code block 执行)
    ↓
! 行 1: 上报上次的 last_result.json + 发 start ping (确定性)
    ↓
LLM 执行 Phase 0-5，生成 HTML
    ↓
Stop hook: 检测 .running → 如果 LLM 已写 last_result.json 则跳过，否则写基础版 → 删 .running (确定性)
    ↓
下次 Preamble ! 行 1: 读 last_result.json → 上报 → 删除 (确定性)
```

#### 首次使用统计引导

如果用户从未被询问过使用统计偏好（`~/.s2h/.telemetry-prompted` 不存在），在 Preamble 结束后、Phase 0 开始前，进行两轮引导。**此引导仅在首次触发，后续运行跳过。**
...
选择你的偏好：
1. community — 开启使用统计（帮助我们了解使用趋势）
2. anonymous — 开启使用统计但不记录 IP
3. off — 完全关闭
```
...
**判断 checklist**（逐条检查，≥2 条命中则归入该层）：

| 层 | 命中条件（≥2 条 → 归入） |
|---|------|
| **Framework** | 1. 含 `preamble`/`telemetry`/`upgrade`/`config`/`setup` 关键词 2. 内容是运维性质（版本检查/环境探测/自动更新） 3. 删除该 section 不影响 skill 的核心业务输出 4. 在 skill 运行的最前或最后执行，不在主流程中间 |
| **Core** | 1. 含 `Phase`/`Step`/`Question`/`Mode` 且描述业务流程 2. 包含用户可见的输入/输出契约 3. 删除该 section 会导致 skill 无法完成主任务 4. 包含条件分支或决策逻辑 |
| **Auxiliary** | 1. 含 `learnings`/`analytics`/`resources`/`handoff` 关键词 2. 内容是辅助功能（经验记录/统计上报/资源推荐） 3. 删除该 section 不影响主任务，但会丢失辅助信息 4. 在主流程完成后才执行 |

**冲突解决**：多层同时 ≥2 条命中 → 优先 Core。都不满足 ≥2 条 → 归为 Core（宁可展开也不要遗漏）。
...
**判断 checklist**（逐条检查，≥2 条命中则归入该结果）：

| 结果 | 命中条件（≥2 条 → 归入） |
|------|------|
| **YES** | 1. 描述用户可感知的业务步骤（Phase/Step/问答/决策） 2. 包含输入/输出契约或数据流 3. 包含条件分支或决策逻辑 4. 包含工具权限声明或安全边界 5. 删除后读者无法理解 skill 的核心工作方式 |
| **NO** | 1. 是内部 AI 指令（语调/反谄媚规则/写作风格/prompt 模板） 2. 是数据格式细节（JSONL schema、config key 枚举） 3. 读者不需要知道也能理解 skill 做什么 4. 内容只对 LLM 执行者有意义，对人类读者无意义 |
| **PARTIAL** | 不满足 YES ≥2 条，也不满足 NO ≥2 条；或者属于 Framework/Auxiliary 层但读者应知其存在 |

**冲突解决**：YES 和 NO 同时 ≥2 条 → 判 YES（宁可展开）。Phase 2.1 的层级分类辅助判断但不直接决定：Core section 如果只是内部 prompt 规范，照样判 NO。
...
```bash
_S2H_TMPL="$HOME/.claude/skills/s2h/templates/base.html"
_S2H_MD_TMPL="$HOME/.claude/skills/s2h/templates/base.md"
```

**硬性要求**：必须 Read `templates/base.html` 和 `templates/base.md`，在模板基础上替换 `{{placeholder}}` 生成最终 HTML。**禁止自行编写 HTML 结构、CSS 样式或 JavaScript**。模板定义了所有的视觉设计、交互逻辑和 JS 功能，LLM 的职责仅限于填充内容。

**两个模板的关系**：`base.md` 定义 Markdown 导出的结构，`base.html` 定义页面结构。LLM 先按 `base.md` 格式生成 Markdown 内容，再将 Markdown 填入 `base.html` 的 `{{markdown_content}}` 占位符，其他占位符也逐一替换。最终产物是单一 HTML 文件。

### 5.2 — 占位符替换（硬模板流程）

Read `templates/base.html` 后，替换以下 `{{placeholder}}`。**不得修改模板中的任何 HTML 标签、CSS 属性或 JavaScript 代码。**

| 占位符 | 来源 | 说明 |
|--------|------|------|
| `{{lang}}` | Language Resolution | `zh` / `en` / `ja` ... |
| `{{s2h_version}}` | Preamble | `S2H_VERSION` 变量值 |
| `{{skill_name}}` | Phase 0 | skill 名称（不含路径） |
| `{{subtitle}}` | Phase 4.1 | 一句话定位（≤40 中文字 / ≤80 英文字符） |
| `{{stat_pills}}` | Phase 4.1 | 统计药丸 HTML，每个格式：`<span class="s2h-stat"><span class="s2h-stat-num">N</span> label</span>` |
| `{{nav_overview}}` 等 | `templates/ui_strings.json` | Read 文件，取 `[lang].nav_*` 值 |
| `{{btn_copy_page}}` | `templates/ui_strings.json` | `[lang].btn_copy_page` |
| `{{btn_download_md}}` | `templates/ui_strings.json` | `[lang].btn_download_md` |
| `{{toast_copied}}` | `templates/ui_strings.json` | `[lang].toast_copied` |
| `{{tooltip_copy_page}}` | `templates/ui_strings.json` | `[lang].tooltip_copy_page` |
| `{{btn_show_more}}` | `templates/ui_strings.json` | `[lang].btn_show_more` |
| `{{footer_built_by}}` | `templates/ui_strings.json` | `[lang].footer_built_by` |
| `{{mermaid_code}}` | Phase 4.2 | Mermaid 图源码（纯文本，不含 ``` 围栏） |
| `{{walkthrough_cards}}` | Phase 4.3 | 所有 YES/PARTIAL 卡片的 HTML（使用模板注释中的 card 结构） |
| `{{trust_content}}` | Phase 4.6 | 风险 badge + findings + assessment 的 HTML |
| `{{security_skipped}}` | `templates/ui_strings.json` | `[lang].security_skipped` |
| `{{context_content}}` | Phase 4.7+4.8 | 相关 skill + 配套文件的 HTML |
| `{{markdown_content}}` | base.md 模板填充结果 | Markdown 副本（供 Copy/Download） |
| `{{view_beacon}}` | Preamble `_S2H_TEL_MODE` | 见下方 |

**`{{view_beacon}}` 替换规则**：
- 当 `_S2H_TEL_MODE` 不为 `off` 时，替换为：
  ```javascript
  ;(function(){var k='s2h_viewed_{{skill_name}}';if(sessionStorage.getItem(k))return;sessionStorage.setItem(k,'1');try{navigator.sendBeacon('https://s2h-telemetry.ltsms86.workers.dev/api/view',JSON.stringify({v:'{{s2h_version}}',skill:'{{skill_name}}',lang:'{{lang}}'}));}catch(e){}})();
  ```
- 当 `_S2H_TEL_MODE` 为 `off` 时，替换为空字符串
...
**硬性要求**：必须 Read `templates/ui_strings.json`，从中取对应语言的值替换 `{{placeholder}}`。**禁止凭记忆或自行翻译 UI 文案。**

```bash
# 读取 UI 文案字典
Read templates/ui_strings.json
# 取 _S2H_LANG 对应的语言对象，替换所有 {{nav_*}}, {{btn_*}}, {{toast_*}}, {{tooltip_*}}, {{risk_*}}, {{label_*}}, {{footer_*}}, {{security_*}} 占位符
```

8 语言内置（en/zh/ja/ko/fr/es/de/pt），每个语言 18 个 key。字典未覆盖的语言 → 从 en 翻译。CSS class 名（`.low` `.medium`）不变，只改可见文本。
...
生成 mermaid 代码后，**必须**按以下步骤校验：

**步骤 1: CLI 校验（优先）**

```bash
which mmdc >/dev/null 2>&1 && _MERMAID_CLI=true || _MERMAID_CLI=false
if [ "$_MERMAID_CLI" = "true" ]; then
  echo "$MERMAID_CODE" | mmdc -i /dev/stdin -o /dev/null 2>&1
  # 退出码 0 = 语法正确，非 0 = 语法错误
fi
```

**步骤 2: 自检 checklist（mmdc 不可用时必做）**

| # | 检查项 | 修复方式 |
|---|--------|---------|
| 1 | 所有含空格/特殊字符/中文的节点标签用 `["..."]` 包裹 | `A["中文标签"]` |
| 2 | 每个 `subgraph` 有对应的 `end` | 逐个配对检查 |
| 3 | 箭头语法正确（`-->`, `---`, `-.->`, `==>`)，无多余空格 | `A --> B` 不写 `A -- > B` |
| 4 | 节点 ID 只含 `[a-zA-Z0-9_]`，无连字符/点号 | `phase_1` 不写 `phase-1` |
| 5 | flowchart 声明行正确（`flowchart TB` / `flowchart LR`） | 不写 `graph` 开头 |
| 6 | 节点总数 ≤ 30（超过则简化，合并次要节点） | 将辅助步骤归入父节点 |

校验失败 → 修复后重新校验，不输出未校验的 mermaid 代码。
...
**硬性要求**：必须 Read `templates/completion.txt`，按模板格式填充变量后输出。**禁止自行编造报告格式。**

```bash
Read templates/completion.txt
# 替换 {placeholder} 后输出
```
...
### Telemetry: 写入 last_result.json（enriched 版，best-effort）

**Stop hook 已保底**：即使此步骤被跳过，`bin/s2h-on-stop.sh` hook 也会写基础版 `last_result.json`。此步骤的价值是写入 hook 无法获取的丰富字段（skill 名、risk、lines）。
...
LASTRESULT
  echo "S2H_LAST_RESULT: written (enriched)"
fi
```
...
| 维度 | 机制 | 可靠性 |
|------|------|--------|
| **Start** | Preamble `!` 语法 | 100%（确定性执行） |
| **Complete** | Stop hook 写 `last_result.json` 基础版 | 100%（hook 确定性执行） |
| **Complete (enriched)** | LLM 写 `last_result.json` 含 skill/risk/lines | best-effort |
| **Incomplete** | 下次 Preamble `!` 检测孤儿 `.running` | 100%（确定性检测） |
| **View** | HTML 内置 `sendBeacon` | 100%（硬模板保证） |

用户可在 `~/.s2h/config` 中设置 `telemetry=off` 关闭使用统计。关闭后不写 `last_result.json`，不发 start ping，生成的 HTML 不含 view beacon。
